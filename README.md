...
Renseigner `DATABASE_URL` (pooler en mode transaction accepté : `prepare: false`) puis `npm run db:migrate`. Le script applique `db/migrations/*.sql` dans l'ordre et mémorise ceux déjà passés (`schema_migrations`) : il est sûr de le relancer à chaque déploiement.
