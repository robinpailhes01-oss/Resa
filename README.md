...
Déploiement Vercel, variables minimales : `DATABASE_URL` (Supabase → Connect → Transaction pooler, port 6543), `RESO_SITE_URL` (`https://votre-domaine`), `INTERNAL_TASKS_SECRET` (chaîne aléatoire). Sans `RESEND_API_KEY` + `EMAIL_FROM`, l'application fonctionne mais aucun email ne part (inscription et réservations restent possibles).
