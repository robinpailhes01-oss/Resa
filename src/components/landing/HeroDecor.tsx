import { Calendar, CalendarCheck, Heart, Leaf, Mail, Scissors, Star, User } from "lucide-react";
...
function Lotus(props: SVGProps<SVGSVGElement>) {
  return <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" {...props}><path d="M12 20c-5-4-5-10 0-16 5 6 5 12 0 16Z" /><path d="M12 20C5 20 2 16 2 10c5 0 9 4 10 10Zm0 0c7 0 10-4 10-10-5 0-9 4-10 10Z" /><path d="M12 20c-4 2-8 1-11-2m11 2c4 2 8 1 11-2" /></svg>;
}
...
export function HeroArcs() {
  ...
  return (
    <>
    <svg aria-hidden="true" className="hero-arcs-mobile pointer-events-none absolute -z-10 sm:hidden" viewBox="0 0 514 370" fill="none">
      {[158, 205, 250].map((radius) => <ellipse key={radius} cx="257" cy="111" rx={radius} ry={radius * 1.28} stroke="#a499e5" strokeWidth="0.65" />)}
    </svg>
    <svg
      aria-hidden="true"
      className="pointer-events-none absolute left-1/2 top-[170px] hidden sm:block -z-10 -translate-x-1/2 -translate-y-1/2 md:top-[200px]"
      width="1700"
      height="1700"
...
const icons: Array<{ icon: Icon; tone: string; className: string; delay: number }> = [
  { icon: Calendar, tone: "text-brand", className: "left-[9%] top-[34px] md:left-[13%] md:top-[100px]", delay: 700 },
  { icon: Lotus, tone: "text-brand", className: "left-[6%] top-[108px] md:left-[7%] md:top-[220px]", delay: 820 },
  { icon: Scissors, tone: "text-[#e78075]", className: "hidden md:flex md:left-[13%] md:top-[335px]", delay: 940 },
  { icon: Heart, tone: "text-[#e78075]", className: "right-[9%] top-[34px] md:right-[13%] md:top-[85px]", delay: 760 },
  { icon: Leaf, tone: "text-[#58a982]", className: "right-[6%] top-[103px] md:right-[7%] md:top-[220px]", delay: 880 },
  { icon: User, tone: "text-brand", className: "right-[10%] top-[179px] md:right-[13%] md:top-[330px]", delay: 1000 },
];
      ...
      {icons.map(({ icon: Icon, tone, className, delay }, i) => (
        <span
          ...
          className={cn(
            ...
            "hero-icon size-10 [&_svg]:size-5 sm:size-10 md:size-14 md:[&_svg]:size-6",
            className,
            tone,
          )}
...
export function FloatingBadgeMobile() {
  ...
  return (
    <div
      aria-hidden="true"
      className="email-badge enter absolute z-10 w-[112px] rounded-xl bg-card px-3 py-2.5 shadow-[0_14px_40px_-12px_rgba(105,80,232,0.28)] ring-1 ring-line/60 sm:hidden"
