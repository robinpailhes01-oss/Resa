...
export function Pricing() {
  ...
  return (
    <Section id="tarif" labelledBy="tarif-title">
      ...
      <div
        id="tarif-card"
        className="reveal mx-auto grid max-w-[720px] overflow-hidden rounded-card bg-card shadow-preview ring-1 ring-line/60 md:grid-cols-2"
      >
        <div className="flex flex-col bg-brand p-6 text-white md:p-8">
          <p className="text-small font-semibold uppercase tracking-[0.12em] text-white/80">{pricing.planName}</p>
          {pricePrefix ? <p className="mt-4 text-small font-medium text-white/80">{pricePrefix}</p> : null}
          <p className="flex flex-wrap items-baseline gap-x-2 text-white">
            ...
            <span className="text-[16px] font-medium text-white/80">{pricing.priceUnit}</span>
          </p>
          <p className="mt-2 text-white/90">{pricing.scope}</p>
          <div className="mt-6 md:mt-auto md:pt-8">
            ...
            <p className="mt-3 text-small text-white/85">{byMode(pricing.afterCta)}</p>
          </div>
        </div>
