...
export async function getNotificationSettings(establishmentId: string, tx: Db = getSql()): Promise<NotificationSettings> {
  const rows = await tx<SettingsRow[]>`select * from notification_settings where establishment_id = ${establishmentId}`;
  if (rows[0]) return mapSettings(rows[0]);
  const [created] = await tx<SettingsRow[]>`insert into notification_settings (establishment_id) values (${establishmentId}) on conflict (establishment_id) do update set updated_at = now() returning *`;
  return mapSettings(created);
}

export async function updateNotificationSettings(establishmentId: string, input: NotificationSettings): Promise<void> {
  await getSql()`
...
export type EmailJobKind = "booking_confirmation" | "booking_cancelled" | "pro_new_booking" | "booking_reminder" | "review_request";
...
export async function scheduleBookingEmails(
  tx: Db,
  params: { establishmentId: string; bookingId: string; startsAt: Date; endsAt: Date; source: "online" | "manual"; hasClientEmail: boolean; manageToken?: string | null },
): Promise<void> {
  const s = await getNotificationSettings(params.establishmentId, tx);
  const jobs: Array<{ kind: EmailJobKind; at: Date }> = [];
  const now = new Date();
  if (params.hasClientEmail && s.confirmationEnabled) jobs.push({ kind: "booking_confirmation", at: now });
  if (params.source === "online" && s.notifyProOnBooking) jobs.push({ kind: "pro_new_booking", at: now });
  if (params.hasClientEmail && s.reminderEnabled) {
    const at = new Date(params.startsAt.getTime() - s.reminderHours * 3600_000);
    if (at.getTime() > now.getTime() + 5 * 60_000) jobs.push({ kind: "booking_reminder", at });
  }
  if (params.hasClientEmail && s.reviewEnabled) {
    jobs.push({ kind: "review_request", at: new Date(params.endsAt.getTime() + s.reviewDelayHours * 3600_000) });
  }
  for (const job of jobs) {
    const carriesToken = job.kind === "booking_confirmation" || job.kind === "booking_reminder";
...
export async function onBookingCancelled(tx: Db, params: { establishmentId: string; bookingId: string; hasClientEmail: boolean; cancelledBy: "client" | "pro" }): Promise<void> {
...
type JobContext = {
  kind: EmailJobKind;
  establishment: { name: string; slug: string; phone: string | null; publicEmail: string | null; addressLine: string | null; postalCode: string | null; city: string | null; timezone: string };
  booking: { id: string; serviceName: string; startsAt: Date; endsAt: Date; durationMin: number; priceCents: number; status: string; manageUrl: string | null };
...
const p = (t: string) => `<tr><td style="padding:12px 28px 0 28px;font-size:16px;line-height:25px;">${t}</td></tr>`;
...
function addressLine(e: JobContext["establishment"]): string | null {
  const parts = [e.addressLine, [e.postalCode, e.city].filter(Boolean).join(" ")].filter(Boolean);
  return parts.length ? parts.join(", ") : null;
}

export function renderJobEmail(ctx: JobContext): EmailMessage | null {
  const { kind, establishment: e, booking: b, client, practitionerName, settings } = ctx;
  ...
  const details = [
    ...
    ...(b.priceCents > 0 ? [detail("Prix", formatPriceCents(b.priceCents))] : []),
  ...
  switch (kind) {
    case "booking_confirmation": {
      ...
      return {
        ...
        html: shell(subject, e.name, [
          ...
          ...details,
          ...(b.manageUrl ? [button(b.manageUrl, "Voir ou annuler mon rendez-vous")] : []),
    ...
    case "review_request": {
      if (!client?.email || b.status === "cancelled" || b.status === "no_show") return null;
      const subject = settings.reviewSubject || "Comment s’est passé votre rendez-vous ?";
      ...
      return {
        ...
        html: shell(subject, e.name, [
          ...
          p(`Nous espérons que vous avez apprécié votre moment chez ${esc(e.name)}.${link ? " Votre avis nous aide à améliorer votre expérience." : ""}`),
    ...
    case "pro_new_booking": {
      const subject = `Nouveau rendez-vous : ${client ? `${client.firstName} ${client.lastName}`.trim() : "client"} · ${b.serviceName}`;
      ...
      return {
        ...
        html: shell(subject, e.name, [
          ...
          detail("Client", client ? `${client.firstName} ${client.lastName}`.trim() : "—"),
...
export async function processEmailJobs(limit = 25, now = new Date()): Promise<{ claimed: number; sent: number; failed: number; skipped: number }> {
  const sql = getSql();
  ...
  try {
    ...
    await alertOps("Fournisseur email non configuré : emails de rendez-vous en attente", { error: String(error) });
    return summary;
  }
  ...
  const jobs = await sql<JobRow[]>`
    ...
    select j.id, j.kind, j.attempts, j.establishment_id, j.booking_id, j.manage_token,
      ...
      e.postal_code as e_postal_code, e.city as e_city, e.timezone as e_timezone,
      ...
      pr.name as p_name, c.first_name as c_first_name, c.last_name as c_last_name, c.email as c_email
    from claimed j
  ...
  for (const job of jobs) {
    try {
      ...
      const needsManageUrl = job.kind === "booking_confirmation" || job.kind === "booking_reminder";
      const manageUrl = !needsManageUrl ? null : job.manage_token ? absoluteUrl(`/rdv/${job.manage_token}`) : job.b_manage_token_hash ? await manageUrlForBooking(job.booking_id) : null;
      const message = renderJobEmail({
        ...
        client: job.c_first_name !== null ? { firstName: job.c_first_name, lastName: job.c_last_name ?? "", email: job.c_email } : null,
      ...
      const next = attempts < MAX_ATTEMPTS ? new Date(now.getTime() + BACKOFF_MS[Math.min(attempts - 1, BACKOFF_MS.length - 1)]) : null;
      if (next) {
        ...
        await alertOps("Email de rendez-vous abandonné après échecs répétés", { jobId: job.id, kind: job.kind });
      }
      summary.failed += 1;
    }
  }
