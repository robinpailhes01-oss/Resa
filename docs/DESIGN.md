...
8. **Avis** : après le tarif, sur un dégradé violet profond → bleu, texte blanc, grande citation abstraite. Tant que `src/content/fr/testimonials.ts` est vide, ce bloc de pré-lancement s’affiche ; dès qu’un témoignage autorisé est ajouté (citation, prénom, établissement, activité, photo facultative), un bandeau d’avis (`src/components/ui/testimonial-marquee.tsx`) le remplace automatiquement : deux rangées de cartes qui défilent en sens inverse (une seule rangée sous quatre avis), pause au survol, fondu sur les bords, initiale en pastille lavande quand il n’y a pas de photo.
...
- `prefers-reduced-motion: reduce` : état final immédiat, aucune transition, aucune dérive, pas d’inclinaison de la fenêtre, mots nets sans flou, navigation sans transition, halo spotlight désactivé, bandeau d’avis immobile.
