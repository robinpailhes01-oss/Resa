...
7. **Tarif** (`src/components/ui/pricing-switch.tsx`) : en-tête centré (pastille « Une offre simple », titre, intro), sélecteur en pilule « Solo / Petite équipe » (onglets accessibles au clavier), puis une carte dans un cadre lavande : à gauche la liste des inclusions du profil choisi, à droite pastille, nom du profil, description, prix 39 € HT / mois, périmètre et CTA. Le prix ne change jamais : le sélecteur ne change que la lecture de l’offre. Le panneau se met à jour avec un court fondu (`.tab-panel-in`) et un liseré lumineux violet parcourt le bord du cadre (`src/components/ui/border-beam.tsx`, `offset-path` + masque limité à la bordure). En dessous : lien vers la FAQ et précisions de transparence.
8. **Avis** : après le tarif. Un bandeau d’avis (`src/components/ui/testimonial-marquee.tsx`) lit `src/content/fr/testimonials.ts` (citation, prénom, établissement, activité, photo facultative) ; la liste contient pour l’instant six avis d’exemple à remplacer par des avis réels. Si elle est vidée, un bloc de pré-lancement sur dégradé violet profond → bleu prend sa place. Le bandeau : deux rangées de cartes qui défilent en sens inverse (une seule rangée sous quatre avis), pause au survol, fondu sur les bords, initiale en pastille lavande quand il n’y a pas de photo.
9. **FAQ** en bas de page (`src/components/landing/Faq.tsx`, ancre `#faq`), en forme de conversation : la question est un message reçu, la réponse un message envoyé en bulle sombre qui se déplie en dessous, avec un horodatage. La même FAQ accompagne le formulaire sur `/preinscription`.
10. **Pied de page** léger : logo, liens de sections et liens légaux. Le formulaire de préinscription vit sur `/preinscription`, ouverte par tous les CTA avec les paramètres de campagne.
...
- Bandeau d’avis : défilement CSS linéaire continu (9 s par carte, 24 s minimum), pause au survol et au focus ; en mouvement réduit, rangée fixe que l’on fait défiler soi-même.
- Tarif : changement de profil en 360 ms (flou 2 px → net, 14 px vers le haut) ; liseré de 90 px qui fait le tour du cadre en 7 s, en continu.
...
- `prefers-reduced-motion: reduce` : état final immédiat, aucune transition, aucune dérive, pas d’inclinaison de la fenêtre, mots nets sans flou, navigation sans transition, halo spotlight désactivé, bandeau d’avis immobile, liseré du tarif masqué et changement de profil sans animation.

Inspirations : les motifs « Morphing Scroll Navbar », « Text Blur Reveal », « Soft Blur In », « Spotlight Card », « How It Works Steps », « FAQ Chat Accordion » et « Testimonial Marquee » publiés sur 21st.dev ont servi de référence ; ils ont été réécrits en CSS et React natifs (aucune dépendance ajoutée, aucun code de bibliothèque tierce).
